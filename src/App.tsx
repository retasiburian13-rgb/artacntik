  ...
  useEffect(() => {
    ...
    recognition.current.onresult = (e: any) => {
      ...
      if (transcript.includes("sensor") || transcript.includes("suhu") || transcript.includes("kelembapan")) {
         ...
         if (isTurnOn) {
            if (isRelay1) { if (!stateRef.current.relays[1]) toggleRelay(1); speakText("Relay satu dinyalakan"); matched = true; }
            ...
            else if (isPattern1) { if (!stateRef.current.patterns[1]) togglePattern(1); speakText("Pola satu dinyalakan"); matched = true; }
            else if (isPattern2) { if (!stateRef.current.patterns[2]) togglePattern(2); speakText("Pola dua dinyalakan"); matched = true; }
         } 
         else if (isTurnOff) {
            if (isRelay1) { if (stateRef.current.relays[1]) toggleRelay(1); speakText("Relay satu dimatikan"); matched = true; }
            ...
            else if (isPattern1) { if (stateRef.current.patterns[1]) togglePattern(1); speakText("Pola satu dimatikan"); matched = true;}
